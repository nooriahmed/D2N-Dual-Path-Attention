...
from tensorflow.keras.layers import Input, SeparableConv2D, BatchNormalization, PReLU, MaxPooling2D, UpSampling2D, Concatenate, Conv2D, Multiply, Dropout, Add, Lambda
from tensorflow.keras.models import Model
import tensorflow as tf
...
class GradientReversalLayer(tf.keras.layers.Layer):
    ...
    def call(self, inputs, **kwargs):
        ...
        def grad_reverse(x):
            def grad(dy):
                return -self.lambda_ * dy  # Reverse gradient during backpropagation
            return x, grad
...
def domain_classifier(feature_map):
    # Simple fully connected classifier for domain prediction
    ...
    x = PReLU()(x)
    x = MaxPooling2D(pool_size=(2, 2))(x)
    x = Conv2D(1, (1, 1), activation='sigmoid')(x)  # Binary domain classifier output
    return x
...
def dual_path_unet_with_da(input_shape=(60, 60, 3), dropout_rate=0.5, lambda_=1.0):
    ...
    pool1_1, pool1_2 = cross_domain_attention(pool1_1, pool1_2)
    pool2_1, pool2_2 = cross_domain_attention(pool2_1, pool2_2)
    ...
    conv3 = SeparableConv2D(256, (3, 3), padding='same')(merged)
    ...
    # Domain Classifier (Adversarial Loss)
    grl_layer = GradientReversalLayer(lambda_=lambda_)(conv3)
    domain_output = domain_classifier(grl_layer)

    # Build Model
    model = Model(inputs, [outputs, domain_output])
...
def plot_history(history):
    ...
    plt.subplot(1, 2, 2)
    # Access accuracy for the first output (segmentation output)
    plt.plot(history.history['output_1_accuracy'], label='Train Accuracy') # This was originally output_1_accuracy
    plt.plot(history.history['val_output_1_accuracy'], label='Val Accuracy') # This was originally val_output_1_accuracy
    plt.title('Accuracy')
    plt.xlabel('Epoch')
    plt.ylabel('Accuracy')
    plt.legend()
